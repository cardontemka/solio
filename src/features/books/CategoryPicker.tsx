...
import { BOOK_CATEGORY, BOOK_CATEGORY_MAX, CATEGORY_LABEL } from '@/types/domain'
 ...
 * to the ones in use plus a first screenful.
 */
export function CategoryPicker({
  ...
  const visible = expanded
    ? BOOK_CATEGORY
    : BOOK_CATEGORY.filter((c, i) => i < 12 || chosen.includes(c))
  ...
  return (
    <div className={styles.wrap}>
      ...
      {BOOK_CATEGORY.length > visible.length || expanded ? (
        <button
          type="button"
          className={styles.more}
          onClick={() => setExpanded((v) => !v)}
        >
          {expanded ? 'Хураах' : `Бүх ангилал (${BOOK_CATEGORY.length})`}
        </button>
      ) : null}
